...
namespace MidiSheetMusic
{
    ...
    public class SymbolWidths
    {
        ...
        private Dictionary<int, float> maxwidths;
                            ...
                            List<LyricSymbol>[] tracklyrics)
        {
            ...
            widths = new Dictionary<int, float>[tracks.Length];
            for (int track = 0; track < tracks.Length; track++)
            {
                widths[track] = GetTrackWidths(tracks[track]);
            }
            maxwidths = new Dictionary<int, float>();

            /* Calculate the maximum symbol widths */
            foreach (Dictionary<int, float> dict in widths)
            {
        ...
        private static Dictionary<int, float> GetTrackWidths(List<MusicSymbol> symbols)
        {
            Dictionary<int, float> widths = new Dictionary<int, float>();
         ...
         */
        public float GetExtraWidth(int track, int start)
        {
